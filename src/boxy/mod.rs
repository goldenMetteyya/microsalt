...
fn internal_lock(message: &[u8], nonce: [u8; curve25519xsalsa20poly1305::NONCE_LEN], pk: &PublicKey, sk: &SecretKey) -> Box {
        ...
        curve25519xsalsa20poly1305::box_(&mut cipher, &m, &nonce, &sk.0, &pk.0);
...
//     let c = internal_lock(&m, nonce, &bobpk, &alicesk);
     
//     // let mut key = [0u8;32];
//     // curve25519xsalsa20poly1305::box_beforenm(&mut key, &bobpk.0, &alicesk.0);

//     // //   let mut cipher = Vec::with_capacity(m.len() + curve25519xsalsa20poly1305::ZERO_LEN);
       


//     // let mut cpre = Vec::with_capacity(m.len());
//     // cpre.resize(32, 0u8);
//     // //cpre.extend_from_slice(&m[..]);
//     // // let m = cipher.clone();
//     // curve25519xsalsa20poly1305::box_afternm(&mut cpre, &m, &nonce, &key);
